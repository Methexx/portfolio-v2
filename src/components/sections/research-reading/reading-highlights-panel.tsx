...
type ReadingHighlightsPanelProps = {
  bodySlot?: ReactNode;
  className?: string;
  highlightsSlot?: ReactNode;
  metadataSlot?: ReactNode;
  tagRowSlot?: ReactNode;
  titleSlot?: ReactNode;
};
...
export function ReadingHighlightsPanel({
  ...
  tagRowSlot,
  titleSlot,
}: ReadingHighlightsPanelProps) {
  return (
    <div
      className={cn(
        "rounded-[1.7rem] border border-research-border bg-research-panel p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.7)] sm:p-6",
        className,
      )}
    >
      {metadataSlot ?? (
