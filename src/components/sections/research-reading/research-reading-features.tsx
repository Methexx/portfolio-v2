...
import { researchReadingFeatures } from "@/components/sections/research-reading/research-reading-data";

type ResearchReadingFeaturesProps = {
  itemsSlot?: ReactNode;
};

export function ResearchReadingFeatures({ itemsSlot }: ResearchReadingFeaturesProps) {
  return (
    <div className="mt-10 grid grid-cols-1 gap-x-10 gap-y-8 border-t border-border/70 pt-8 md:grid-cols-2 md:pt-10">
      {itemsSlot ??
        researchReadingFeatures.map(({ title, description, icon: Icon }) => (
